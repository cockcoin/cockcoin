...
B4ARGS="-datadir=$D4"
$COCKCOIND $COCKCOINDARGS $B4ARGS &
B4PID=$!
...
B3ARGS="-datadir=$D3"
$COCKCOIND $COCKCOINDARGS $B3ARGS &
B3PID=$!
...
function S {
  ...
  if [[ $TXID == "" ]] ; then
      echoerr "node${1}: error sending ${3} ckc"
...
function StartThree {
  $COCKCOIND $COCKCOINDARGS $B1ARGS &
  B1PID=$!
  $COCKCOIND $COCKCOINDARGS $B2ARGS &
  B2PID=$!
  $COCKCOIND $COCKCOINDARGS $B3ARGS &
  B3PID=$!
}
